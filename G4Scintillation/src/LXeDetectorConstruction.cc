...
void LXeDetectorConstruction::DefineMaterials()
{
  ...
  fPDMS->AddElement(fSi, natoms=1);
  fPDMS->AddElement(fC, natoms=2);
  fPDMS->AddElement(fH, natoms=6);
  fPDMS->AddElement(fO, natoms=1);
  ...
  std::vector<G4double> lxe_Energy = {2.0 * eV, 2.3 * eV, 2.6 * eV};

  std::vector<G4double> lxe_SCINT = {0.1, 1.0, 0.1};
  std::vector<G4double> lxe_RIND = {2.2, 2.2, 2.2};
  std::vector<G4double> lxe_ABSL = {1.0 * cm, 1.0 * cm, 1.0 * cm};
  ...
  fLXe_mt->AddConstProperty("SCINTILLATIONTIMECONSTANT1", 8. * ns);
  ...
  // Set RINDEX for PDMS
  fPDMS_mt = new G4MaterialPropertiesTable();
  fPDMS_mt->AddProperty("RINDEX", lxe_Energy, {1.41, 1.41, 1.41});
  fPDMS->SetMaterialPropertiesTable(fPDMS_mt);

  // Set RINDEX for Ag
  fAg_mt = new G4MaterialPropertiesTable();
  fAg_mt->AddProperty("RINDEX", lxe_Energy, {0.27, 0.27, 0.27});
  fAg->SetMaterialPropertiesTable(fAg_mt);

  // Set RINDEX for SiPM
  fSiPM_mt = new G4MaterialPropertiesTable();
  fSiPM_mt->AddProperty("RINDEX", lxe_Energy, {1.55, 1.55, 1.55});
  fSiPM->SetMaterialPropertiesTable(fSiPM_mt);
  ...
  auto fair_mt = new G4MaterialPropertiesTable();
  fair_mt->AddProperty("RINDEX", lxe_Energy, {1.7, 1.7, 1.7});
  fAir->SetMaterialPropertiesTable(fair_mt);

  auto vacuum_mt = new G4MaterialPropertiesTable();
  vacuum_mt->AddProperty("RINDEX", lxe_Energy, {1., 1., 1.});
...
G4VPhysicalVolume* LXeDetectorConstruction::Construct()
{
  ...
  if (fMainVolumeOn) {
    fMainVolume = new LXeMainVolume(nullptr, G4ThreeVector(), fExperimentalHall_log, false, 0, this);
  }
...
void LXeDetectorConstruction::ConstructSDandField()
{
  ...
  if (!fSiPM_SD.Get()) {
    ...
    auto sipmSD = new LXeSiPMSD("/LXeDet/sipmSD");
    fSiPM_SD.Put(sipmSD);
  }
  G4SDManager::GetSDMpointer()->AddNewDetector(fSiPM_SD.Get());
  SetSensitiveDetector(fMainVolume->GetLogSiPM(), fSiPM_SD.Get());
}
...
void LXeDetectorConstruction::SetDefaults()
{
  ...
  fScint_x = 6.02 * mm; //was 17.8 * cm
  fScint_y = 6.03 * mm; //was 17.8 * cm
  fScint_z = 2.72 * mm; //was 22.6 * cm
